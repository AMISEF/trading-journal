...
17. [لیگ تریدرها (Traders League)](#۱۷-لیگ-تریدرها-traders-league)
...
### ۳.۱ سرویسِ ماشین‌به‌ماشین برای پنل ادمینِ ربات الگو هاب

بک‌اندِ این ریپو یک روترِ کوچک به نامِ `/api/service/*` دارد
(`backend/app/api/service.py`) که برای هیچ کاربرِ نهایی نیست — فقط توسطِ
**ربات تلگرام «الگو هاب»** (در ریپوی `portfolio`) صدا زده می‌شود تا از داخلِ
ربات بتوان اشتراکِ کاربرانِ همین سایت را فعال کرد و گزارشِ عملکردش را گرفت،
بدونِ آنکه ربات مستقیماً به این دیتابیسِ PostgreSQL وصل شود.

احراز هویت با هدرِ `X-Service-Token` انجام می‌شود که باید دقیقاً برابرِ
متغیرِ زیر باشد:

| متغیر | این ریپو | ریپوی `portfolio` |
|---|---|---|
| توکنِ مشترک | `SERVICE_TOKEN` (در `backend/.env`) | `JOURNAL_SERVICE_TOKEN` (در `portfolio/.env`) |
| آدرسِ این سرویس | — | `JOURNAL_API_BASE` (در `portfolio/.env`؛ روی این سرور: `http://127.0.0.1:8001`) |

اگر `SERVICE_TOKEN` خالی باشد، کلِ این API درخواست‌ها را رد می‌کند و پنلِ
ادمینِ ربات فقط برای بخش‌های مربوط به این سایت پیامِ خطای روشن می‌دهد؛ خودِ
سایتِ ژورنال و بقیهٔ اندپوینت‌ها بی‌اثر و بدون تغییر باقی می‌مانند.
...
| `SERVICE_TOKEN` | توکنِ سرویسِ ماشین‌به‌ماشین برای پنلِ ادمینِ ربات الگو هاب — رجوع به «۳.۱» |
...
## ۱۷. لیگ تریدرها (Traders League)

یک لیدربردِ رقابتی که کاربران را بر اساس نتیجهٔ **واقعیِ ژورنالشان** در یک بازهٔ
زمانیِ شمسی رتبه‌بندی می‌کند. مسیرها:

| بخش | جا |
|---|---|
| صفحه | `frontend/src/app/league/page.tsx` (منوی «لیگ تریدرها») |
| API | `GET /api/league` و `GET /api/league/meta` (`backend/app/api/league.py`) |
| محاسبات | `backend/app/services/league.py` |
| تقویم شمسی | `backend/app/services/jalali.py` |

**بازه‌ها** — روزانه، هفتگی، ماهانه (پیش‌فرض)، فصلی و سالانه؛ همه شمسی. هفته از
**شنبه** شروع و **جمعه** تمام می‌شود. هر بازه یک `key` پایدار دارد
(`1404-05-09` روزانه/هفتگی، `1404-05` ماهانه، `1404-Q2` فصلی، `1404` سالانه) که
دکمه‌های «دورهٔ قبل/بعد» با آن کار می‌کنند.

**معیارها** — درصد سود (پیش‌فرض)، امتیاز لیگ، سود دلاری، حجم معاملات، میانگین
اهرم، حداکثر افت سرمایه (کمتر = بهتر)، ضریب سود، وین‌ریت، میانگین R، بلندترین
رگبار برد، روزهای سبز، انضباط چک‌لیست و تعداد معاملات.

**قواعد**

- مبنای درصدِ سود، موجودیِ کاربر در **لحظهٔ شروع بازه** است؛ سرمایهٔ بزرگ‌تر
  به‌تنهایی مزیت نمی‌سازد.
- فقط چرخهٔ سرمایهٔ فعال حساب می‌شود (بعد از ریست ماهانه، معاملاتِ ماهِ قبل اثر
  ندارند).
- وین‌ریت از `dashboard_stats.win_rate` می‌آید؛ معاملات سربه‌سر در مخرج نیستند.
- کمتر از `league.MIN_TRADES` معاملهٔ بسته‌شده در دوره ⇒ «خارج از رقابت»؛ حذف
  نمی‌شود ولی بعد از واجدین شرایط می‌آید.
- ادمین‌ها و حسابِ دمو در لیگ نمی‌آیند.
- **حریم خصوصی:** برای کاربر عادی فقط نام کاربری برگردانده می‌شود؛ `userId` تنها
  در پاسخِ ادمین پر می‌شود تا بتواند پروفایل کاملِ آن تریدر را باز کند.

**امتیاز لیگ** = بازده (۴۵) + وین‌ریت (۱۵) + ضریب سود (۱۵) + کنترل افت سرمایه
(۱۵) + انضباط چک‌لیست (۱۰)، از ۱۰۰.

### پروفایل تریدر برای ادمین

`/admin/users/<id>` دو تب دارد: «داشبورد کامل» و «ژورنال و مدیریت». تبِ داشبورد
همان کامپوننتِ `frontend/src/components/DashboardView.tsx` را رسم می‌کند که
صفحهٔ داشبوردِ خودِ کاربر هم از آن استفاده می‌کند، پس ادمین **عیناً** همان
داشبورد را می‌بیند و دو نسخهٔ واگرا نگه‌داری نمی‌شود. با `?tab=dashboard` یا
`?tab=manage` می‌توان مستقیم به یکی از تب‌ها لینک داد — ردیف‌های لیگ همین کار را
می‌کنند.
