...
/**
 * اجزای بصریِ داشبورد (کارت‌ها، نمودارها، تقویم سود/زیان) و خودِ نمای داشبورد.
 * صفحهٔ `/dashboard` و پنل ادمین هر دو از همین‌جا رسم می‌شوند.
 */
import {
  Area,
  AreaChart,
...
/**
 * نمای کاملِ داشبورد — دقیقاً همان چیزی که کاربر در «داشبورد» می‌بیند.
 *
 * این کامپوننت هیچ داده‌ای نمی‌گیرد و هیچ حالتی ندارد: فقط یک `DashboardData`
 * را رسم می‌کند. به همین دلیل هم صفحهٔ داشبوردِ کاربر از آن استفاده می‌کند و هم
 * پنل ادمین، پس ادمین عیناً همان داشبوردِ کاربر را می‌بیند و دو نسخهٔ واگرا از
 * یک صفحه نگه‌داری نمی‌شود.
 *
 * `header` (سربرگ و دکمه‌ها) و `footer` (مثلاً فهرست معاملاتِ دمو) از بیرون
 * تزریق می‌شوند، چون در هر مصرف‌کننده فرق می‌کنند.
 */
