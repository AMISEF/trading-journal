...
interface Props {
  ...
  /** When set, shows a chat box; `send` posts a message and returns the thread. */
  chat?: { send: (message: string) => Promise<AIAnalysis> };
}

const POLL_MS = 4000;
const MAX_POLLS = 60; // ~4 minutes, then re-enable with a retry hint

export function AICoachPanel({ fetcher, generator, title, subtitle, pdf, chat }: Props) {
  ...
  const apply = (res: AIAnalysis) => {
    ...
    if (Array.isArray(res.chat)) setChatMsgs(res.chat);
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      setChatError(resp?.data?.detail ?? `خطای سرور (${resp?.status ?? "?"})`);
      // drop the optimistic user bubble on failure
      setChatMsgs((prev) => prev.filter((m, i) => !(i === prev.length - 1 && m.role === "user")));
      setChatInput(msg);
    } finally {
      setChatBusy(false);
    }
        ...
        <div className="space-y-2 rounded-xl border border-border bg-surface-2/30 p-3">
          ...
          {chatMsgs.length > 0 && (
            ...
            >
              ...
              {chatBusy && (
                <div className="flex justify-end">
                  <div className="flex items-center gap-2 rounded-2xl border border-border bg-surface px-3 py-2 text-xs text-muted">
          ...
          <div className="flex items-end gap-2">
            <textarea
              ...
              placeholder="سوالت را درباره‌ی بهبود معاملاتت بپرس…"
              className="tj-input max-h-32 min-h-[40px] flex-1 resize-y text-sm"
            />
            <button
              type="button"
              onClick={() => void sendChat()}
              disabled={chatBusy || !chatInput.trim()}
              className="shrink-0 rounded-xl bg-gradient-to-l from-violet-600 to-sky-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50"
            >
              ارسال
            </button>
          </div>
